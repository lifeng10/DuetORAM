    ...
    for (int i = opt->start; i < opt->end; i++)
    {
        ...
        for (int j = 0; j < (BUCKET_SIZE*(H+1)); j++)
        {
            ...
            if (opt->sharedVector[j] == 1)
            {
                ...
                // Debug: log which slot was selected (only for first chunk to avoid spam)
                if (i == 0) {
                    cout << "    [DotProduct] Selected slot " << j << " (layer " << (j/BUCKET_SIZE) << ", pos " << (j%BUCKET_SIZE) << "), value=" << opt->data_vector[i][j] << endl;
                }
            }
        }
    }
...
int ServerDuetORAM::evict(zmq::socket_t& socket)
{
    ...
    //     {
    //         std::rotate(receivedPuncturedPermutation + i*SIZE_PI, receivedPuncturedPermutation + i*SIZE_PI + SIZE_PI - circularShift_1[i], receivedPuncturedPermutation + i*SIZE_PI + SIZE_PI);
    //     }
    //     for (int j = 0; j < SIZE_PI; j++)
    //     {
    //         std::rotate(fullPermutation + j*SIZE_PI, fullPermutation + j*SIZE_PI + SIZE_PI - circularShift_2[j], fullPermutation + j*SIZE_PI + SIZE_PI);
    //     }
    // }
    // if (this->serverNo == 1)
    // {
    //     for (int i = 0; i < SIZE_PI; i++)
    //     {
    //         std::rotate(fullPermutation + i*SIZE_PI, fullPermutation + i*SIZE_PI + SIZE_PI - circularShift_1[i], fullPermutation + i*SIZE_PI + SIZE_PI);
    //     }
    //     for (int j = 0; j < SIZE_PI; j++)
    //     {
    //         std::rotate(receivedPuncturedPermutation + j*SIZE_PI, receivedPuncturedPermutation + j*SIZE_PI + SIZE_PI - circularShift_2[j], receivedPuncturedPermutation + j*SIZE_PI + SIZE_PI);
    //     }
    // }
    ...
    // ThreadSocketArgs recvThread_args;   // Add for passing server object
    // recvThread_args.socket = recvSocket;   // Add
    // cout << "   [server listens xored dot product vector] Creating Threads for Receiving vector..." << endl;
    ...
    // recvThread_args.sockData = &recvSocket_arg;     // Add
    // pthread_create(&thread_recv, NULL, &ServerDuetORAM::thread_socket_func, (void*)&recvThread_args);
    // cout << "   [server listens dot product vector] Received xored dot product vector!" << endl;
    // //===============================================================================

    // //===========FOR SENDING=========================================================
    // struct_socket sendSocket_arg;
    // ThreadSocketArgs sendThread_args;       // Add for passing server object
    // sendThread_args.socket = sendSocket;      //  Add
    ...
    // sendThread_args.sockData = &sendSocket_arg;     // Add
    // // pthread_create(&thread_send, NULL, &ServerTripORAM::thread_socket_func, (void*)&sendSocket_arg);
    // pthread_create(&thread_send, NULL, &ServerDuetORAM::thread_socket_func, (void*)&sendThread_args);
